...
def run_audit(*, fluent_csv: Path, f3_csv: Path, out_dir: Path, alpha_deg: float, mach: float):
    ...
    f3_w = f3["side_id"] == 1
    f3_l = f3["side_id"] == 0
    f3_pe_w = f3['p_e_Pa'][f3_w]
    f3_pe_l = f3['p_e_Pa'][f3_l]
    print(f"\n--- Faceted3D basic stats ---")
    print(f"  windward: {int(np.sum(f3_w))} pts, p_e range [{float(np.nanmin(f3_pe_w)):.1f}, {float(np.nanmax(f3_pe_w)):.1f}] Pa")
    if np.any(np.isfinite(f3_pe_l)):
        print(f"  leeward:  {int(np.sum(f3_l))} pts, p_e range [{float(np.nanmin(f3_pe_l)):.1f}, {float(np.nanmax(f3_pe_l)):.1f}] Pa")
    else:
        print(f"  leeward:  {int(np.sum(f3_l))} pts, p_e all NaN (expected — leeward model does not compute edge pressure)")
    ...
    # Freestream pressure
    try:
        from ref_enthalpy_method.atmosphere.ussa1976 import ussa1976_0_32km
        p_inf_est, rho_inf_est, T_inf_est = ussa1976_0_32km(h_m=30000.0, R_gas_J_per_kgK=287.0)
    except Exception:
        p_inf_est = 1190.0  # fallback
    print(f"  Estimated p_inf at 30km: {p_inf_est:.1f} Pa")
    ...
    for i in range(flt.shape[0]):
        ...
        aligns.append({
            ...
            "p_fluent_Pa": float(flt[i, 3]),
                "q_fluent_W_m2": -float(flt[i, 4]),  # Fluent q negative (wall heat loss), flip to positive
    ...
    # Compute Fluent Cp on centerline for comparison with Faceted3D
    # Cp = (p - p_inf) / (0.5 * rho_inf * V_inf^2)
    try:
        from ref_enthalpy_method.atmosphere.ussa1976 import ussa1976_0_32km as _us
        _p_inf, _rho_inf, _T_inf = _us(30000.0, 287.0)
    except Exception:
        _p_inf, _rho_inf, _T_inf = 1190.0, 0.0184, 226.5
    _v_inf = float(mach) * float(np.sqrt(1.4 * 287.0 * _T_inf))
    _q_dyn = 0.5 * _rho_inf * _v_inf ** 2
    ...
    if np.any(cl_mask):
        ...
        cl_q3 = q_f3[cl_mask]
        cl_cp_fluent = (cl_pf - _p_inf) / _q_dyn
        cl_cp_f3 = (cl_p3 - _p_inf) / _q_dyn
        ...
        cl_qf = cl_qf[order]
        cl_q3 = cl_q3[order]
        cl_cp_fluent = cl_cp_fluent[order]
        cl_cp_f3 = cl_cp_f3[order]
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        p3_w = p_f3[w_mask]
        qf_w = q_fluent[w_mask]
        q3_w = q_f3[w_mask]
        if np.any(w_mask):
            f.write("## Q4: p_e & q magnitude comparison (windward)\n\n")
            f.write("### Pressure\n\n")
            ...
            f.write(f"\n### Heat flux (Fluent sign flipped: -q → +q)\n\n")
            f.write(f"| Metric | Fluent (W/m²) | Faceted3D (W/m²) | Ratio F3/Fluent |\n")
            f.write(f"|--------|---------------|------------------|----------------|\n")
            f.write(f"| Mean | {float(np.nanmean(qf_w)):.1f} | {float(np.nanmean(q3_w)):.1f} | {float(np.nanmean(q3_w)/max(float(np.nanmean(qf_w)),1)):.3f} |\n")
            f.write(f"| Min | {float(np.nanmin(qf_w)):.1f} | {float(np.nanmin(q3_w)):.1f} | — |\n")
            f.write(f"| Max | {float(np.nanmax(qf_w)):.1f} | {float(np.nanmax(q3_w)):.1f} | — |\n")
            f.write(f"\n**Judgment**: ")
        ...
        if np.any(cl_mask):
            ...
            f.write(f"Freestream: p_inf={_p_inf:.1f} Pa, q_dyn={_q_dyn:.1f} Pa\n\n")
            f.write(f"| x (m) | Fluent p | F3 p_e | Fluent Cp | F3 Busemann cp | Fluent q | F3 q |\n")
            f.write(f"|-------|----------|--------|-----------|----------------|----------|-------|\n")
            n_display = min(8, cl_x.size)
            step = max(1, cl_x.size // n_display)
            for i in range(0, cl_x.size, step):
                f.write(f"| {cl_x[i]:.4f} | {cl_pf[i]:.1f} | {cl_p3[i]:.1f} | {cl_cp_fluent[i]:.4f} | {cl_cp_f3[i]:.4f} | {cl_qf[i]:.1f} | {cl_q3[i]:.1f} |\n")
            if cl_x.size > 0:
                f.write(f"| {cl_x[-1]:.4f} | {cl_pf[-1]:.1f} | {cl_p3[-1]:.1f} | {cl_cp_fluent[-1]:.4f} | {cl_cp_f3[-1]:.4f} | {cl_qf[-1]:.1f} | {cl_q3[-1]:.1f} |\n")
            f.write(f"\n**Judgment**: ")
        ...
        if np.any(w_mask):
            ...
            f.write(f"Mean q_fluent (windward): {float(np.nanmean(qf_w)):.1f} W/m²\n")
            f.write(f"Mean q_f3 (windward): {float(np.nanmean(q3_w)):.1f} W/m²\n")
