...
- Fluent clean：完成。
- LF clean：完成。
- LF clean → Fluent clean mapping：未完成。
- mapping QA：未完成。
...
- 当前不能宣称 mapping complete、temperature error 已可计算或 leeward model validated。
...
- 在 LF clean → Fluent clean mapping 与 mapping QA 完成前不计算或发布背风温度误差。
