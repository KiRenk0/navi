...
> 更新：2026-07-21（N3a.3c candidate manifest tooling 已实现并验证）
...
## 8. N3a.3 Candidate Manifest Tooling 当前状态（2026-07-21）

- N3a.3a exact-LF restoration=`complete`。
- N3a.3b contract/design audit=`complete`。
- N3a.3c tooling implementation and validation=`complete`。
- candidate manifest tooling prerequisite 已关闭；`tpg-candidate-manifest/v1` 的 API/CLI 入口已经实现并验证。
- M8/30 尚未运行，尚未生成真实 candidate manifest；M8/30 仍不是正式 case，未加入 `CASES`、registry 或 baseline，未进行 promotion。
- N3a 尚未退出，不返回 GATE A；strategy v1.2 保持 frozen / unchanged。M6/40 仍等待用户数据或精确授权。
- Git closeout 完成后，下一步也不是自动运行 M8/30；只能由后续监督 GPT 在获得独立授权时安排 M8/30 candidate generation。
