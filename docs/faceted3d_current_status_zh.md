...
- 当前 production source identity：schema=`git-head-tree-source-identity/v1`，count=`69`，`inventory_paths_sha256=0cb3a5aa592256807a9e975e9213867c2b6b8337902450209a5421845e37edf8`，`aggregate_sha256=92114f8e2d798d3f6e574a68a710233463c0d30a4396a8839bf190dcb21cf38d`。
...
- 最新 N3a.3c closeout validation：195 tests PASS（另有 117 subtests PASS）；
- overall：`CURRENT REGRESSION OVERALL: PASS`。

唯一 current regression 命令为 `python scripts/tools/current_baseline_regression_check.py`。该 PASS 只证明 harness 实际执行的数值、字段、semantic QA、endpoint/metadata、source identity 与 artifact integrity gate，不得扩大解释为未执行的 validation 或性能裁决。

### 3.1 Source identity promotion

source identity promotion 只收口源码身份，不等于数值 baseline freeze。Phase 5A Fluent clean 与 Phase 5B1 LF clean 均属于 additive geometry/mapping infrastructure。`fluent_clean.py`、公开 `semantic_valid_mask()` 和 `lf_clean.py` 均不进入 `fields.npz`，不增加 72-field schema，不改变 Groups 1–8 数值。Phase 5B1 收口时 source inventory 为 58 项；当前回归以实际 manifest inventory 执行 source integrity gate 并保持 PASS。
...
- Fluent clean → LF clean pairing、wall-temperature ingestion、source-level comparison 与正式 evidence chain 已完成；当前仍无统一性能 threshold 或 provider 路线裁决，不得声称背风温度模型已经 CFD validated。
...
- 正式 pairing implementation：已由后续 Phase 5C 完成。
- mapping QA implementation：已完成。
- wall-temperature ingestion：已由后续 Phase 5D 完成。
- leeward source-level error calculation：已由后续 Phase 5E 完成；未建立性能 PASS/FAIL。
...
当前最新门禁为 195 tests、117 subtests、schema v5、72 fields、Groups 1–8、source integrity、artifact integrity 与 `CURRENT REGRESSION OVERALL: PASS`。Phase 5C–5E 已完成 pairing、ingestion 与 source-level comparison；这些结果仍不构成 provider route decision、统一性能 threshold 或 leeward model validation。
...
- runner 复现 provenance 使用正式参数 `--T_inf_K` / `--p_inf_Pa`。explicit manifest 交叉校验 `inputs.T_inf_K_override`、`inputs.p_inf_Pa_override`、`freestream.freestream_source`、`freestream.T_inf_K` 与 `freestream.p_inf_Pa`；source 必须为 `explicit_override`，并记录 `atmosphere.explicit_freestream_override=true`。显式 summary 缺少 provenance pair 时拒绝生成 manifest。
- 非显式 candidate 路径保持向后兼容并记录 `explicit_freestream_override=false`；candidate manifest 顶层字段集合未改变。正式 v5 baseline、`CASES`、registry、freeze/check、source inventory、Groups 1–8 和 72-field contract 均未改变。
- 独立 QA 证据：candidate 专项 `73 passed`；full suite `235 passed`、`117 subtests passed`；两个正式 baseline regression、72 fields、Groups 1–8、source integrity 与 artifact integrity 均为 PASS；正式 baseline、evidence、两个 v5 manifest 与历史 pyc 均为 zero drift。测试通过不表示 candidate 数值结果已经生成或验证。
