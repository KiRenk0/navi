...
- source identity promotion 只管理源码身份，不等于数值 baseline freeze；当前 55 项 source hash 全部 PASS。STL parsed geometry、CSV parsed numeric arrays、`local_incidence.py` AST/executable semantics、`fields.npz`、summary、schema、artifact hashes 与 Groups 1–8 数值均未改变。
...
- 本 exact-projection 合同只覆盖 Fluent geometry input、坐标合同、canonical identity 与 exact STL projection；Phase 4A 已另行冻结 projected-point raw geometry semantics，但真实 21,250 点 semantics integration、Fluent/LF clean、LF→Fluent mapping 与 temperature error 均未完成。
...
- Phase 4A projected-point geometry semantics 纯层已完成；真实 21,250 个 Fluent canonical points 的 semantics integration、Fluent/LF clean、LF→Fluent mapping 与 temperature error 均未开始。
- 下一阶段唯一入口是 Phase 4B：将 `FluentSurfaceProjection` 接入 projected geometry semantics，生成 raw semantic fields 并执行真实几何统计 QA；仍不构造 clean。
- 只有未来另行重启 residual learning 时，才需要单独冻结 residual-label mapping、dataset schema 和 case-level validation protocol。
- windward TPG 基线、大气、pressure 和 edge-state 已冻结；baseline schema v5、72 fields、Groups 1–8 零漂移，84 tests 与 55 项 source identity 全部 PASS。
