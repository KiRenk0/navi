...
- source identity promotion 只管理源码身份，不等于数值 baseline freeze；当前 57 项 source hash 全部 PASS。STL parsed geometry、CSV parsed numeric arrays、`local_incidence.py` AST/executable semantics、`fields.npz`、summary、schema、artifact hashes 与 Groups 1–8 数值均未改变。
...
- 本 exact-projection 合同只覆盖 Fluent geometry input、坐标合同、canonical identity 与 exact STL projection；Phase 4A projected-point raw geometry semantics、Phase 4B 真实 21,250 点 projected semantics integration 与 Phase 5A Fluent clean 均已另行冻结。LF clean、LF→Fluent mapping 与 temperature error 尚未完成。
...
- Phase 5A 已完成：Fluent clean 是 raw projected semantics 上的 geometry-only 只读派生 subset，不回写 raw semantic fields。
- 下一阶段唯一入口为 LF clean；随后才是 LF clean → Fluent clean mapping、mapping QA、wall-temperature ingestion 与 leeward temperature error。
- LF clean、LF→Fluent mapping、wall-temperature ingestion 与 temperature error 均未完成。
- 只有未来另行重启 residual learning 时，才需要单独冻结 residual-label mapping、dataset schema 和 case-level validation protocol。
- windward TPG 基线、大气、pressure 和 edge-state 已冻结；baseline schema v5、72 fields、Groups 1–8 零漂移，97 tests、77 subtests 与 57 项 source identity 全部 PASS。
