...
> 更新：2026-07-20（Phase 5E Source-level Comparison QA 收口）
...
- 架构边界保持：geometry pairing → mapping diagnostics → physical comparison。当前 comparison 只冻结 source identity、direct indexing 与误差公式；freestream-recovery provider 仍是 diagnostic，是否升级等待下一轮正式性能误差证据。
