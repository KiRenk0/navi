...
- source identity promotion 只管理源码身份，不等于数值 baseline freeze；当前 53 项 source hash 全部 PASS。STL parsed geometry、CSV parsed numeric arrays、`local_incidence.py` AST/executable semantics、`fields.npz`、summary、schema、artifact hashes 与 Groups 1–8 数值均未改变。
...
- 本合同只覆盖 Fluent geometry input、坐标合同、canonical identity 与 exact STL projection；投影三角面几何语义、upper/lower、surface class、normal-source、任意投影点 `x/c`/`y/b`、Fluent/LF clean、LF→Fluent mapping 与 temperature error 均未完成。
...
- Fluent geometry input、坐标合同、canonical identity、exact STL projection 和全量 5 mm gate 已正式收口；Fluent/LF clean、LF→Fluent mapping 与 temperature error 尚未开始。
- 下一阶段唯一入口是投影三角面的几何语义与可复用坐标参数化；在 clean 与 mapping 完成前不计算温度误差。
- 只有未来另行重启 residual learning 时，才需要单独冻结 residual-label mapping、dataset schema 和 case-level validation protocol。
- windward TPG 基线、大气、pressure 和 edge-state 已冻结；baseline schema v5、72 fields、Groups 1–8 零漂移，74 tests 与 53 项 source identity 全部 PASS。
