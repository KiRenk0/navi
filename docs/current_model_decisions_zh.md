...
- source identity promotion 只管理源码身份，不等于数值 baseline freeze；当前 56 项 source hash 全部 PASS。STL parsed geometry、CSV parsed numeric arrays、`local_incidence.py` AST/executable semantics、`fields.npz`、summary、schema、artifact hashes 与 Groups 1–8 数值均未改变。
...
- 本 exact-projection 合同只覆盖 Fluent geometry input、坐标合同、canonical identity 与 exact STL projection；Phase 4A projected-point raw geometry semantics 与 Phase 4B 的真实 21,250 点 projected semantics integration 已另行冻结。Fluent/LF clean、LF→Fluent mapping 与 temperature error 均未完成。
...
- Phase 4B 已完成：exact projection、projected semantics adapter 与真实 21,250 点 deterministic geometry-only QA 已收口。
- 下一阶段唯一入口是 Fluent clean。Fluent clean 只能是 raw geometry/physics fields 上的派生 subset；clean 不得回写或改变 raw semantic fields。
- Fluent/LF clean、LF→Fluent mapping、wall-temperature ingestion 与 temperature error 均未完成。
- 只有未来另行重启 residual learning 时，才需要单独冻结 residual-label mapping、dataset schema 和 case-level validation protocol。
- windward TPG 基线、大气、pressure 和 edge-state 已冻结；baseline schema v5、72 fields、Groups 1–8 零漂移，93 tests、59 subtests 与 56 项 source identity 全部 PASS。
